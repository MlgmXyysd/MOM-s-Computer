...
function Ball(_x,_y,_state,_link){
    ...
    this.__defineSetter__("location",function(val){
        val.x = Math.max(Math.min(val.x,1010),0);
        val.y = Math.max(Math.min(val.y,2000),0);
        this._location = val;
...
function init() {
    ...
    balls = [];
    links = [];
    balls.push(new Ball(500, 1900, "fixed"));
    balls.push(new Ball(450, 2000, "fixed"));
    balls.push(new Ball(550, 2000, "fixed"));

    for (var x = 0; x < 30; ++x) {
        balls.push(new Ball(20+x*33, 1600, "active"));
    }
...
function physics(){
    ...
    // $("#max_height").html ( "Height: <b>" + Math.round(max_height/10) / 10) + " meter(s)</b>";
...
function CreateBall(x,y) {
	if (iscd=="0") {
		balls.push(new Ball(x,y,"active"));
		iscd="1"
		setTimeout(function() {iscd="0"},1000)
	} else {
		return false;
	}
}
...
function toMOM() {
	var height=Math.round(max_height/10) / 10;
	if (height<15) {
		$("#popup").show();
		$("#block").show();
	} else {
		$("#mom-splash").show();
		window.location="MOM.en.html";
	}
}
...
$("#create").hover(function(){
         $("#application").css("top","45px");
         $("#application").css("left","-33px");
         $("#application").css("font-size","17px");
},function(){
         $("#application").css("top","35px");
         $("#application").css("left","-30px");
         $("#application").css("font-size","15px");
})